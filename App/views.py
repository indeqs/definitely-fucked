...
class EmergencyContactListView(View):
    def get(self, request):
        ...
        return render(
            request, "App/emergency_contact_list.html", {"contacts": contacts}
        )
...
def emergency_contact_list(request):
    ...
    contacts = EmergencyContact.objects.filter(user=request.user)
    return render(request, "App/emergency_contact_list.html", {"contacts": contacts})


def success_view(request):
    return render(request, "App/success.html")  # Create this template
...
def add_emergency_contact(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            emergency_contact.user = request.user  # Assign the user
            emergency_contact.save()
            return redirect("success_url")  # Redirect after saving
    else:
        form = EmergencyContactForm()

    return render(request, "App/add_emergency_contact.html", {"form": form})


@login_required
def emergency_contacts_list(request):
    contacts = EmergencyContact.objects.filter(user=request.user)
    return render(request, "App/emergency_contacts_list.html", {"contacts": contacts})
...
class ApprovedAlertListView(ListView):
    model = Alert
    template_name = (
        "App/approved_alerts.html"  # The template where alerts will be rendered
    )
...
def ussd_callback(request):
    ...
    logger.info(
        "Parameters: sessionId=%s, serviceCode=%s, phoneNumber=%s, text=%s",
        session_id,
        service_code,
        phone_number,
        text,
        ...
        if len(input_steps) == 1:
            ...
            if len(input_steps) == 2:
                ...
                if password != confirm_password:
                    ...
                    registration_success = register_user(
